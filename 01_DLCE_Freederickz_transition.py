...
Lx = 0.34e-3 # m
Ly = Lx/10# m
domain = mesh.create_rectangle(MPI.COMM_WORLD, [[0, 0], [Lx, Ly]], [10, 4], mesh.CellType.quadrilateral)
...
for rate_phi in [1.0]:      # list of all the considered rates of applying the load/voltage/stretch
    ...
    for theta_00_deg in theta_list:
        ...
        for rrr in r_list:
            ...
            mi_0 = Constant(domain, 0.7e6)                      # elastic branch shear modulus  ----- Povećan za 7.5%
            mi_a = Constant(domain, PETSc.ScalarType(1/7*mi_0)) # anisotropic shear modulus     ----- 
            ...
            a_0 = Constant(domain, 0.075)                       #         ----- Povećan za 21%
            a_p = Constant(domain, 0.0)                         #
            m = Constant(domain, 0.3)                           #         Smanjen za 18%
            eta0 = Constant(domain, 0.8e6)                      #         Povećan za 25%
            k_1 = Constant(domain, 0.44e6)                      #         Smanjen za 7%
            k_2 = Constant(domain, 0.1)                         #         Dodat nelinearni efekat
            ...
            Vmax = 12000.00 #[V]                            # maximum considered voltage (electric potential)
            ...
            T3 = functionspace(domain, T0) # Tensor function spacea
            ...
            def T_R_nceq_calc(u,p,teta):
                ...
                T_anis = 2*k_1*(I_4bar-1)*exp(k_2*(I_4bar-1)**2)*(outer(Fbar*dd0,Fbar*dd0) - 1/3*inner(outer(Fbar*dd0,Fbar*dd0),Id)*Id) * inv(F.T)
                T_R_nceq0 = mi_0 * (g_0*K0bar + abar*Z0bar - \
                    n_f_norm2bar*a_tbar**2*(m/a_0)*B0bar)*inv(F.T) - p*J*inv(F.T)
                T_R_nceq = T_R_nceq0 + T_anis #+ Tdis
            ...
            # -------------------------------------------------------
            # results file name
            results_name = "2D_LCE_fredericksz_transition__r={}__".format(rrr)
            ...
            file_results = VTXWriter(
                MPI.COMM_WORLD,
                "results/rate={}/".format(rate_phi) + results_name + ".bp",
        ...
        # --------------   BCs for Fredericksz transition  --------------------------------
            bcs_11 = dirichletbc(0.0, Bottom_u2_dofs, ME.sub(0).sub(1)) 

            bcs_12 = dirichletbc(phi_cons, Top_q_dofs, ME.sub(8))  # phi ramp - Top
            bcs_13 = dirichletbc(0.0, Bottom_q_dofs, ME.sub(8))  # phi ground - Bottom

            bcs = [bcs_11, bcs_12, bcs_13]
        # --------------------------------------------------------------------------------

        # --------------   BCs for bending  ----------------------------------------------
            # bcs_21 = dirichletbc(0.0, Left_u1_dofs , ME.sub(0).sub(0)) 
            # bcs_22 = dirichletbc(0.0, Left_u2_dofs , ME.sub(0).sub(1)) 

            # bcs_23 = dirichletbc(phi_cons, Top_q_dofs, ME.sub(8))  # phi ramp - Top
            # bcs_24 = dirichletbc(0.0, Center_q_dofs, ME.sub(8))    # phi ground - Bottom
            # bcs_25 = dirichletbc(0.0, Bottom_q_dofs, ME.sub(8))    # phi ground - Bottom

            # bcs = [bcs_21, bcs_22, bcs_23, bcs_24, bcs_25]
        # --------------------------------------------------------------------------------

        # --------------   BCs for buckling 1---------------------------------------------
            # bcs_31 = dirichletbc(0.0, Left_u1_dofs , ME.sub(0).sub(0)) 
            # bcs_32 = dirichletbc(0.0, Left_u2_dofs , ME.sub(0).sub(1)) 

            # bcs_33 = dirichletbc(0.0, Right_u1_dofs , ME.sub(0).sub(0)) 
            # bcs_34 = dirichletbc(0.0, Right_u2_dofs , ME.sub(0).sub(1)) 

            # bcs_35 = dirichletbc(phi_cons, Top_q_dofs, ME.sub(8))  # phi ramp - Top
            # bcs_36 = dirichletbc(0.0, Center_q_dofs, ME.sub(8))    # phi ground - Bottom
            # bcs_37 = dirichletbc(0.0, Bottom_q_dofs, ME.sub(8))    # phi ground - Bottom

            # bcs = [bcs_31, bcs_32, bcs_33, bcs_34, bcs_35, bcs_36, bcs_37]
        # --------------------------------------------------------------------------------


            # Set up thr nonlinear problem
...
# ====================================================================================
            ...
            csv_dir_path = "results/rate={}/csvfiles_".format(rate_phi)+analysis_type
            if not os.path.exists(csv_dir_path):
                os.makedirs(csv_dir_path)
            
            with open(csv_dir_path+"/Fredericksz_transition__r={}__theta0=5__timehistory_0___t.csv".format(rate_phi, rrr), 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(timeHist0[0:ind])
            csvfile.close()
            
            with open(csv_dir_path+"/Fredericksz_transition__r={}__theta0=5__timehistory_1___phi.csv".format(rate_phi, rrr), 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(timeHist1[0:ind])
            csvfile.close()

            with open(csv_dir_path+"/Fredericksz_transition__r={}__theta0=5__timehistory_2___lambda1.csv".format(rate_phi, rrr), 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(timeHist2[0:ind])
            csvfile.close()

            with open(csv_dir_path+"/Fredericksz_transition__r={}__theta0=5__timehistory_3___theta.csv".format(rate_phi, rrr), 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
...
# ====================================================================================
            ...
            plt.tight_layout()
            if analysis_type=="theta_fixed":
                plt.savefig(figs_dir_path+"/Fig_Fredericksz_transition____rate={}____r__theta0={}___lambda-phi.png".format(rate_phi, theta_00_deg), dpi=600)
            if analysis_type=="r_fixed":
                plt.savefig(figs_dir_path+"/Fig_Fredericksz_transition____rate={}____theta0__r={}___lambda-phi.png".format(rate_phi, rrr), dpi=600)
...
# ====================================================================================
            ...
            # Control number of major ticks (4-6)
            from matplotlib.ticker import MaxNLocator, AutoMinorLocator
            ax_theta_phi.xaxis.set_major_locator(MaxNLocator(nbins=4))  # 5 ticks (including ends)
            ax_theta_phi.yaxis.set_major_locator(MaxNLocator(nbins=4))

            # Add minor ticks
            ax_theta_phi.xaxis.set_minor_locator(MaxNLocator(nbins=4))
            ax_theta_phi.yaxis.set_minor_locator(MaxNLocator(nbins=4))
            ...
            # Optional: Format tick labels (e.g., 2 decimal places)
            ax_theta_phi.xaxis.set_major_formatter(plt.FormatStrFormatter('%.0f'))
            ...
            if analysis_type=="theta_fixed":
                plt.savefig(figs_dir_path+"/Fig_Fredericksz_transition____rate={}____r__theta0={}___theta-phi.png".format(rate_phi, theta_00_deg), dpi=600)
            if analysis_type=="r_fixed":
                plt.savefig(figs_dir_path+"/Fig_Fredericksz_transition____rate={}____theta0__r={}___theta-phi.png".format(rate_phi, rrr), dpi=600)
